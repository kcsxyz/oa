...
<body>
	...
	<script type="text/javascript">
		...
		function create_dept_table(result) {
			//清空table
			$("#attendData").empty();
			var attends = result.extend.pageInfo.list;
			$
					.each(
							attends,
							function(index, item) {
								...
								var attendMorLeaveTd = $("<td></td>").append(
										timestampToTime(item.attendMorLeave));
								var attendNoonStartTd = $("<td></td>").append(
										timestampToTime(item.attendNoonStart));
								var attendNoonLeaveTd = $("<td></td>").append(
										timestampToTime(item.attendNoonLeave));
								var weekTd = $("<td></td>").append(
										item.week);
								var statusTd; 
								
								if(item.status==1){
									statusTd= $("<td></td>").append("正常");
								}else{
									statusTd= $("<td></td>").append("异常");
								}
								var timeTd = $("<td></td>").append(
										timeToTime(item.currDate));
