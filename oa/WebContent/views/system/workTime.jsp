...
<body>
					...
					<table
						...
						<thead>
							<tr class="bg-primary">
								...
								<th style="text-align: center;">上午上班打卡时间</th>
								<th style="text-align: center;">上午工作时间</th>
								<th style="text-align: center;">上午下班打卡时间</th>
								<th style="text-align: center;">下午上班打卡时间</th>
								<th style="text-align: center;">下午工作时间</th>
	...
	<script type="text/javascript">
		...
		function create_dept_table(result) {
			//清空table
			$("#deptData").empty();
			var workTime = result.extend.workTimeList;
			$
					.each(
							workTime,
							function(index, item) {
								...
								var attendMorStartTd = $("<td></td>").append(
										timestampToTime(item.attendMorStartTime)+"-"+timestampToTime(item.attendMorEndTime));
								...
								var attendNoonEndTd = $("<td></td>").append(
										timestampToTime(item.attendAfterLeaveStartTime)+"-"+timestampToTime(item.attendAfterLeaveEndTime));
								var workNoonTd = $("<td></td>").append(
										timestampToTime(item.workStartTimeAfternoon)+"-"+timestampToTime(item.workEndTimeAfternoon));
								...
								$("<tr></tr>").append(checkBoxTd).append(idTd)
										.append(attendMorStartTd)
										.append(workMorTd)
										.append(attendMorEndTd).append(attendNoonStartTd).append(workNoonTd).append(attendNoonEndTd)
												.append(btnTd)
										.appendTo("#deptData");
							});
		}
	...
	/* -------------------部门添加---------------------- */
		...
		function create_dept(ele){
			...
			//获取部门
			$.ajax({
				url: "${pageContext.request.contextPath}/attend/workTimeList",
				type: "get",
				success:function(result){
